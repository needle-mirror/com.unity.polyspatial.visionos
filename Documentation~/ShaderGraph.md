...
## Supported targets
The `Universal` and `Built-In` targets are supported for conversion.  For both targets, the `Lit`, `Unlit`, and `Canvas` materials are supported (`Sprite Unlit` is treated the same as `Unlit`), as well as the `Opaque` and `Transparent` surface types, the `Alpha Clipping` setting, the `Render Face` option, and the `Depth Write` option (where `Auto` is interpreted as `Enabled`).  The `Depth Test` setting may be `LEqual` or `Always`.  For `Transparent` surfaces, the `Alpha`, `Premultiply`, and `Additive` blending modes are supported.  The `Cast Shadows` setting is supported in the `Universal` target.  No other target settings are currently supported for conversion.  All materials will have tangent space fragment normals.

Note that using the `Canvas` material type does not provide all the functionality of the UI image shader graph (`Packages/PolySpatial/Resources/Shaders/MaskingShader.shadergraph`), such as hover transitions and viewport clipping.  To create custom UI shaders with those features, copy `MaskingShader.shadergraph` and modify it.

### Allow Material Override
The `Allow Material Override` target setting is supported for the `Render Face`, `Depth Write`, `Depth Test`, and `Cast Shadows` settings (only), allowing you to change those settings via material properties (versus having to use a different shader), subject to the limitations described in the `Supported targets` section (`Depth Test` must be `LEqual` or `Always`).  Switching between `Opaque` and `Transparent` surface types, between different blending modes, or toggling `Alpha Clipping` via material overrides is not currently supported.
...
|         | Branch on Input Connection       | <span style="color: green; font-weight: bold;">&#x2713; Supported</span>                                                                                                             |
