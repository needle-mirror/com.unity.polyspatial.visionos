...
| **Auto-Create Volume Camera**           | When **enabled**, PolySpatial creates a volume camera automatically if there is no volume camera after scene load. **Disable** this property if you create the initial volume camera from a script after the scene loads. This option exists to improve first-touch ease-of-use, but we ***recommend developers create their volume camera manually*** once they have a clear sense of how their application interacts with visionOS. **Note*:* that if you first load a scene without a volume camera with this option enabled, then additively load a scene without a volume camera, the auto-created volume camera will be destroyed in favor of the new manually created volume camera. |
